...
public class AuthorizationController {
	...
	public String authorization_code_grant(Model model) {
		System.out.println("-------/authorize  " + this.messagingClientAuthCodeRestTemplate.getOAuth2ClientContext().getAccessToken());
		System.out.println("-------/authorize  " + this.messagingClientAuthCodeRestTemplate.getOAuth2ClientContext().getAccessTokenRequest());
	...
	public String authorized(Model model) {
		System.out.println("-------/authorized ---"  + this.messagingClientAuthCodeRestTemplate.getOAuth2ClientContext().getAccessToken());
		System.out.println("-------/authorize  " + this.messagingClientAuthCodeRestTemplate.getOAuth2ClientContext().getAccessTokenRequest());
		String[] messages = this.messagingClientAuthCodeRestTemplate.getForObject(this.messagesBaseUri, String[].class);
		model.addAttribute("messages", messages);
		return "index";
	...
	public String password_grant(Model model, HttpServletRequest request) {
		...
		OAuth2AccessToken accessToken = this.messagingClientPasswordRestTemplate.getAccessToken();
        if (accessToken != null) {
